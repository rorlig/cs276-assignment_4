...
public class Util {
    ...
    static String[] features = {"url", "title", "body", "header", "anchor"};
    ...
    public static double[] getInstance(Query q, Document d, Map<String, Double> idfs, Map<String, Map<String, Double>> rel){
      ...
      Map<String,Map<String, Double>> typeQueryScore=getDocTermFreqs(d,q);
      Map<String, Double> queryScore=getQueryTermFreqs(q,idfs);
      for(int i=0;i<features.length;i++){
          Map<String, Double> docScores=typeQueryScore.get(features[i]);
          for(String token:docScores.keySet()){
              double score=1D;
              if (idfs.keySet().contains(token))
                  score=idfs.get(token);
              if (docScores.get(token)!=0)
                  score=score*(1+Math.log(docScores.get(token)));
              docScores.put(token,score);
          }
          typeQueryScore.put(features[i],docScores);
      }
      for (int j=0;j<features.length;j++){
          instance[j]=dotProduct(queryScore,typeQueryScore.get(features[j]));
      }
      if(rel!=null){
          instance[features.length]=rel.get(q.query).get(d.url);
      } else {
    ...
    private static Map<String, Double> getQueryTermFreqs(Query q, Map<String, Double> idfs){
        Map<String, Double> tfQuery=new HashMap<>();
        for(String word: new HashSet<String>(q.queryWords)){
            tfQuery.put(word, idfs.keySet().contains(word)?idfs.get(word):1D * (1+Math.log(getCount(word,q))));
        }
        return tfQuery;
    }

    private static int getCount(String token, Query q){
        int count=0;
    ...
    private static double dotProduct(Map<String, Double> queryScore, Map<String, Double> tQueryScore){
        double result=0D;
        for(String word:queryScore.keySet()){
            result+=queryScore.get(word)*tQueryScore.get(word);
        }
        return result;
    }
  */
